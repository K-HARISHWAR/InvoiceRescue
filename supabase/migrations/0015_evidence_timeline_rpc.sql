-- 0015_evidence_timeline_rpc.sql

-- Returns a unified chronological timeline of events for an invoice.
...
BEGIN
    -- Check authorization (assuming the user has access if they can call this, or apply RLS indirectly. We will rely on the fact this RPC is called by an authenticated user who has access to the invoice).
    IF NOT EXISTS (
        SELECT 1 FROM invoices i
        JOIN business_members bm ON bm.business_id = i.business_id
        WHERE i.id = p_invoice_id AND bm.user_id = auth.uid()
    ) THEN
